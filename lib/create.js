...
const generator = require("./generator"); // 模版插入
...
class Creator {
  ...
  async getCollectRepo() {
    ...
    if (collectTemplateNameList.length > 0) {
      ...
      console.log(logSymbols.success, "选择了模版：" + choiceTemplateName);
      this.downloadTemplate(choiceTemplateName);
    } else {
      console.log(logSymbols.error, "暂无模版，请确认后重试");
    }
  }
  ...
  async downloadTemplate(choiceTemplateName) {
    const targetDirectory = path.join(cwd, this.projectName);
    const tempDirectory = path.join(targetDirectory, ".download-temp");
    if (!fs.existsSync(targetDirectory)) {
    ...
    loading.start();
    await this.downloadGitRepo(templateUrl, tempDirectory);
    loading.succeed();

    await generator({
      projectRoot: this.projectName,
      downloadTemp: tempDirectory,
      metadata: {
